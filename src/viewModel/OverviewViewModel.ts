...
export class OverviewViewModel extends BaseViewModel {
  ...
  orgCount() {
    const thiz_ = this;
    this.addDisposable(
      OverviewService.orgCount(),
      new (class extends DisposableObserver<OrgCountBean> {
  ...
  taskCount(endTime: string, startTime: string, timeType: number) {
    const thiz_ = this;
    this.addDisposable(
      OverviewService.taskCount(endTime, startTime, timeType),
      new (class extends DisposableObserver<TaskCountBean> {
  ...
  homeStatistics(endTime: string, startTime: string, timeType: number) {
    const thiz_ = this;
    this.addDisposable(
      OverviewService.homeStatistics(endTime, startTime, timeType),
      new (class extends DisposableObserver<HomeStatisticsBean> {
  ...
  newTask() {
    const thiz_ = this;
    this.addDisposable(
      OverviewService.newTask(),
      new (class extends DisposableObserver<Array<NewTaskBean>> {
